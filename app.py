...
if st.session_state['calc_done']:
    ...
    # Show Report with Internal Print Button
    st.components.v1.html(html_report, height=800, scrolling=True)

else:
    st.info("👈 กรุณากรอกข้อมูลทางด้านซ้าย แล้วกดปุ่ม 'Run Calculation'")import streamlit as st
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import matplotlib.patches as patches
...
def flexureSectionResponse(As_mm2, fc, fy, bw, d, Es=200000, eps_cu=0.003):
    ...
    for i in range(50):
        ...
        a_new = (As_mm2 * fs_new) / (0.85 * fc * bw)
        if abs(fs_new - fs) < 0.1 and abs(a_new - a) < 0.1:
            fs = fs_new; a = a_new; break
        fs = fs_new; a = a_new; c = a / beta1
    c = a / beta1
    eps_t = eps_cu * (d - c) / c if c > 0 else 0.005
    phi = phiFlexureFromStrain(eps_t)
    T = As_mm2 * fs
...
def solve_required_as(Mu_Nmm, As_min, As_max, fc, fy, bw, d):
    ...
    for _ in range(50):
        As_mid = 0.5 * (As_lo + As_hi)
        r = flexureSectionResponse(As_mid, fc, fy, bw, d)
        if r['phiMn'] >= Mu_Nmm: As_hi = As_mid
        else: As_lo = As_mid
    return As_hi
...
def process_calculation(inputs):
    ...
    for case in MuCases:
        ...
        row(f"{title} Mu", "-", "-", f"{fmt(Mu_tfm,3)}", "tf-m", "")
        row(f"{title} Prov", f"Use {barKey}", f"{n}-{barKey}", "OK" if overall else "NO", "-", "OK")
        row(f"{title} Check", "φMn ≥ Mu", f"{fmt(rProv['phiMn']/9.8e6,3)}", "PASS" if passStr else "FAIL", "tf-m", "PASS")
    ...
    for case in VuCases:
        ...
        row(f"{loc} Vu", "-", "-", f"{fmt(Vu_tf,3)}", "tf", "")
        status_shear = "OK" if passStr else "NO"
        row(f"{loc} Provide", f"min(req, {fmt(s_avmin,0)})", "-", f"{stirKey}@{fmt(s_sel/10,0)}cm", "-", status_shear)
...
def create_beam_section(b, h, cover, top_n, bot_n, stir_txt, m_db, s_db, title, bar_name):
    ...
    ax.add_patch(rect)
    margin = cover + s_db/20
    rect_s = patches.Rectangle((margin, margin), b-2*margin, h-2*margin, linewidth=2, edgecolor='#2E7D32', facecolor='none', linestyle='-')
    ax.add_patch(rect_s)
    ...
    def draw_row(n, y, color):
        ...
        dia = m_db/10
        if n == 1: xs = [b/2]
        else: xs = np.linspace(margin + dia/2, b - margin - dia/2, n)
        for x in xs:
            circle = patches.Circle((x, y), radius=dia/2, edgecolor='black', facecolor=color)
            ax.add_patch(circle)
